<template>
  <div class="c-time-picker">
    <div class="input-wrapper">
      ...
      <c-icon name="time" class="icon" style="pointer-events: none;"></c-icon>
      <c-icon name="error" class="icon clear" v-if="value" @click="clearValue" style="color: rgba(0,0,0,0.35);"></c-icon>
    </div>
      ...
      <div class="picker" ref="picker">
        ...
        <div class="picker-items">
          <div class="item" ref="hour">
            <ul class="hour">
              <li v-for="(n, index) in 24" :key="n" @click="handleClick(index, 'hour')" :class="{ selected: hour === index }">
...
import tweenScroll from '@/components/others/tween/tweenScroll.js'
  ...
  computed: {
    value() {
      if (this.hour >= 0 && this.minute >= 0 && this.second >= 0) {
        return `${fillZeroInTime(this.hour)}:${fillZeroInTime(this.minute)}:${fillZeroInTime(this.second)}`
      } else {
        return ''
      }
    }
