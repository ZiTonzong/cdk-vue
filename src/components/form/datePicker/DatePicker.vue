<template>
  <div class="c-date-picker">
    ...
    <c-sperad :visible="pickerVisible" :duration="500" class="spread">
      <div class="picker" ref="picker">
        ...
        <div class="action-bar">
          <div class="action-bar">
            ...
            <div class="time">
              <span class="year" @click="showList('yearListVisible')" v-show="dateListVisible || monthListVisible">
                {{ year }}年
              </span>
              <span class="month" @click="showList('monthrListVisible')" v-show="dateListVisible">
                {{ year }}年
              </span>
            ...
            <div class="picker-body" v-show="dateListVisible">
              ...
              <div class="dates">
                <div class="row">
                  <span v-for="n in dateArray.slice(0, 7)" :key="n" :class="getDateClassObj(n, 1)" @click="changeDate(n, 'first')">{{ n }}</span>
                </div>
                <div class="row">
                ...
                <div class="row">
                  <span v-for="n in dateArray.slice(28, 35)" :key="n" :class="getDateClassObj(n, 5)" @click="changeDate(n, 'last')">{{ n }}</span>
                </div>
                <div class="row">
                  <span v-for="n in dateArray.slice(35, 42)" :key="n" :class="getDateClassObj(n, 6)" @click="changeDate(n, 'last')">{{ n }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="month-list" v-show="monthListVisible">
              <div class="row" v-for="(monthList, index) in zhCNMonthList" :key="index">
                <span :class="{selected: month === monthObj.num}" v-for="monthObj in monthList" :key="monthObj.monthText" @click="setMonth(monthObj.num)">
                  {{ monthObj.monthText }}
                </span>
              </div>
            </div>
            <div class="year-list" v-show="yearListVisible">
              ...
              <div class="row">
                <span v-for="n in yearArray.slice(9, 12)" :key="n" @click="setYear(n)" :class="{sub: n > yearAreaBase + 9, selected: year === n}">
                  {{ n }}
                </span>
              </div>
            </div>
            <div class="century-list" v-if="centuryListVisible">
              <div class="row">
                <span v-for="(n, index) in centuryArray.slice(0, 3)" :key="n" @click="setYearAreaBase(n, index)" :class="{ sub: index === 0, selected: +n.substring(0, 4)===yearAreaBase }">
                  {{ n }}
                </span>
              </div>
              <div class="row">
                <span v-for="n in centuryArray.slice(3, 6)" :key="n" @click="setYearAreaBase(n)" :class="{ selected: +n.substring(0, 4)===yearAreaBase }">
                  {{ n }}
                </span>
              </div>
              <div class="row">
                <span v-for="n in centuryArray.slice(6, 9)" :key="n" @click="setYearAreaBase(n)" :class="{ selected: +n.substring(0, 4)===yearAreaBase }">
                  {{ n }}
                </span>
              </div>
              <div class="row">
                <span v-for="(n, index) in centuryArray.slice(9, 12)" :key="n" @click="setYearAreaBase(n, index)" :class="{ sub: index === 2, selected: +n.substring(0, 4)===yearAreaBase }">
                  {{ n }}
                </span>
              </div>
            </div>
...
export default {
  ...
  data () {
    return {
      ...
      yearAreaBase: 0,
      century: 0,
      zhCNMonthList: [[{monthText: '一月', num: 1}, {monthText: '二月', num: 2}, {monthText: '三月', num: 3}], [{monthText: '四月', num: 4}, {monthText: '五月', num: 5}, {monthText: '六月', num: 6}], [{monthText: '七月', num: 7}, {monthText: '八月', num: 8}, {monthText: '九月', num: 9}], [{monthText: '十月', num: 10}, {monthText: '十一月', num: 11}, {monthText: '十二月', num: 12}]]
    }
  },
  computed: {
    dateArray () {
